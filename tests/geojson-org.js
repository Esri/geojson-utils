...
function geoJsonTest(obj) {
     ...
     */
    /*asyncTest('Add Converted Features to Map', function () {
        var map,
            handle;
...
function testReverseRings() {
    test('Converting single GeoJSON geometry of type MultiPolygon with reversed rings to EsriJSON', function (){
        var geojson =  {
            "next_feature" : "1",
        ...
        //reverse rings for comparison
        var poly = geojson.features[0].geometry.coordinates;
        var clockwiseRings = [];
        ...
        var esriJson = root.geoJsonConverter().toEsri(geojson);
        notEqual(typeof esriJson, 'undefined', 'EsriJson not null');
        equal(poly.length, esriJson.features[0].geometry.rings.length, 'EsriJson has same number of rings as GeoJson');
        deepEqual(poly, esriJson.features[0].geometry.rings,  'EsriJson has ring coordinates in reverse order of GeoJson');
    });
}
